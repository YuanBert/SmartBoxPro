...
LOCKERRORCODE BSP_CanInit(void)
{
	...
	//»ñÈ¡CanIdÖµ
	if((uint32_t)GPIO_PIN_SET == GetAddrBit3Val)
	{
		CanId |= 0x08;
	}
	if((uint32_t)GPIO_PIN_SET == GetAddrBit2Val)
	{
		CanId |= 0x04;
	}
	if((uint32_t)GPIO_PIN_SET == GetAddrBit1Val)
	{
		CanId |= 0x02;
	}
	if((uint32_t)GPIO_PIN_SET == GetAddrBit0Val)
	{
		CanId |= 0x01;
	}
